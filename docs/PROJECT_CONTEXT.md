...
Текущая точка: PR #8 (ANT-5–ANT-11) влит 8 сентября, `origin/main` — `feb0e1a`. Начат Sprint 16 — CI and Docker image publishing, общая ветка `feature/ci-sprint` от этого main. Текущее поручение — только ANT-12: Ruff и pytest в GitHub Actions. Последующие задачи спринта пока не реализуем. Пользователь просит двигаться строго по задачам Linear и объяснять новые понятия на примерах.

ANT-12: добавлен `.github/workflows/ci.yml` для PR в main и push в main: Ubuntu, Python 3.13, установка `.[dev]`, Ruff и pytest. Настройки заданы явно в env без секретов; PostgreSQL не запускается, доступ к БД подменён в существующих тестах. README описывает CI и больше не требует переключения на уже влитую Docker-ветку. Локально 16 тестов проходят, Ruff и `git diff --check` проходят. Повторный запуск из архивной копии исходников без `.env` с очищенным окружением и CI-настройками также дал 16 passed и зелёный Ruff; использованы установленные зависимости проектной .venv. Установка зависимостей на чистом GitHub runner и сам workflow ещё требуют удалённого запуска. Следующий шаг — коммит ANT-12, общий PR спринта и проверка Actions. Статус задачи Linear не закрывался.
